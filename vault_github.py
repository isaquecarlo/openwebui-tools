...
LÃª arquivos do vault Obsidian sincronizado no GitHub - funciona 24/7 no servidor
Autor: VENTURI-AI

CONFIGURAÃ‡ÃƒO:
1. No OpenWebUI, vÃ¡ em Admin â†’ Settings â†’ Environment Variables
2. Adicione: GITHUB_TOKEN = seu_token_aqui
3. O token precisa ter acesso ao repositÃ³rio privado do seu vault
"""
...
class VaultGitHub:
    ...
    def __init__(self, repo: str = "isaquecarlo/obsidian-vault", subpasta: str = "VENTURI-AI", branch: str = "master", token: Optional[str] = None):
        ...
        self.branch = branch
        # Pega token de parÃ¢metro ou variÃ¡vel de ambiente
        self.token = token or os.environ.get("GITHUB_TOKEN") or os.environ.get("GITHUB_PAT")
        if not self.token:
            raise Exception(
                "GITHUB_TOKEN nÃ£o configurado. "
                "VÃ¡ em Admin â†’ Settings â†’ Environment Variables e adicione GITHUB_TOKEN"
            )
        self.base_url = f"https://api.github.com/repos/{repo}"
        self.cache = {}  # Cache simples em memÃ³ria
        self.headers = {
            ...
            if response.status_code == 403:
                ...
                if remaining == "0":
                    ...
                    raise Exception(
                        ...
                        f"Configure GITHUB_TOKEN para mais requisiÃ§Ãµes."
                    )
            
            # Trata outros erros
            if response.status_code == 404:
                raise Exception(f"Arquivo/pasta nÃ£o encontrado: {url}")
            
            if response.status_code == 401:
                raise Exception("Token invÃ¡lido. Verifique se GITHUB_TOKEN estÃ¡ correto.")
    ...
    def _adicionar_prefixo(self, caminho: str) -> str:
        """Adiciona prefixo da subpasta se necessÃ¡rio"""
        if not caminho:
    ...
    def ler_arquivo(self, caminho: str) -> str:
        ...
        caminho_completo = self._adicionar_prefixo(caminho)
        url = f"{self.base_url}/contents/{caminho_completo}?ref={self.branch}"
        data = self._make_request(url, use_cache=False)  # Sempre pega versÃ£o atual
        
        if data.get("type") != "file":
            raise Exception(f"'{caminho}' nÃ£o Ã© um arquivo")
    ...
    def buscar_arquivos(self, query: str, max_resultados: int = 10) -> List[dict]:
        """Busca arquivos por nome ou conteÃºdo"""
        resultados = []
...
class VaultGitHubTools:
    ...
    name = "vault_github"
    description = "Acessa o vault Obsidian via GitHub. LÃª notas, busca conteÃºdo, lista pastas. Funciona 24/7 no servidor sem depender do notebook."
    version = "2.0.0"
    author = "VENTURI-AI"
    ...
    class Tools:
        ...
        def __init__(self):
            # Inicializa com repositÃ³rio padrÃ£o
        ...
        def ler_nota(
            ...
            """LÃª uma nota especÃ­fica do vault Obsidian via GitHub"""
            try:
                # Limpa o caminho
                caminho = caminho_nota.strip("/")
                
                # Adiciona extensÃ£o .md se nÃ£o tiver
                ...
                return {
                    "success": False,
                    "erro": str(e),
                    "sugestao": "Verifique se o caminho estÃ¡ correto ou use 'listar_pasta' para ver os arquivos disponÃ­veis"
                }
        ...
        def buscar_vault(
            ...
            max_resultados: int = Field(
                default=10,
                description="MÃ¡ximo de resultados (padrÃ£o: 10)"
            ),
            __user__: dict = {}
        ):
